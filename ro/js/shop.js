            ...
            <div class="bg-white shadow-sm hover:shadow-md transition-shadow group flex flex-col fade-in cursor-pointer overflow-hidden rounded-xl border border-gray-100" onclick="window.location.href='shop-items/product?id=${p.id}'">
                ...
                <div class="p-6 flex flex-col flex-grow">
                    ...
                    <div class="flex justify-between items-center mt-auto pt-4 border-t border-gray-100">
                        ${(() => {
                            if (p.priceMode === 'from') {
                                return `<span class="text-gold font-serif text-xl font-medium"><span class="text-xs text-gray-500 mr-1 font-normal">de la</span>${p.price} €</span>`;
                            }
                            const isReq = p.priceMode === 'request' || String(p.price) === '0' || p.price === 'Auf Anfrage' || p.price === 'On Request' || p.price === 'La Cerere';
                            if (isReq) {
                                return '<span class="text-gold font-serif text-base font-semibold">La Cerere</span>';
                            }
                        ...
                        <div class="flex space-x-3 items-center">
                            ...
                            ${(() => {
                                ...
                                if (isReq) {
                                    return `
                                    <button onclick="event.stopPropagation(); window.openProductInquiryModal('${p.id}', 'shop')" class="bg-gold hover:bg-black text-white rounded-full w-8 h-8 flex items-center justify-center transition-colors shadow-sm" title="Trimite Cerere">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
                                    </button>`;
                                }
                                return `
                                <button onclick="event.stopPropagation(); addToCart('${p.id}')" class="bg-gray-900 hover:bg-gold text-white rounded-full w-8 h-8 flex items-center justify-center transition-colors shadow-sm" title="Adaugă în coș">
...
function openCheckout() {
    ...
    if (transportLine) {
        if (hasTransport) {
            transportLine.textContent = "Calculat după adresă";
            transportLine.className = "text-xs font-semibold text-amber-600";
        } else {
            transportLine.textContent = "0 € (Gratuit / Ridicare personală)";
