const products = [
    {
        "id": 1,
        "title": "Themen-Dekoration mit Ballonbogen",
        ...
        "shortDesc": "Dekorativer Ballonbogen für Events.",
        "longDesc": "<h2>Beschreibung</h2>\n<p>Verwandeln Sie Ihr Event in eine traumhafte Kulisse mit unserem dekorativen Ballonbogen, perfekt geeignet für Geburtstage, Kinderpartys, Taufen, Jubiläen oder Firmenveranstaltungen.</p>"
    },
    {
        "id": 2,
        "title": "Themen-Dekoration mit Ballonbogen 2",
    ...
    },
    {
        ...
        "shortDesc": "Elegante Dekoration für Events.",
        "longDesc": "<h2>Beschreibung</h2>\n<p>Verwandeln Sie Ihr Event in eine traumhafte Kulisse mit unserer eleganten Dekoration, perfekt geeignet für Geburtstage, Kinderpartys, Taufen, Jubiläen oder Firmenveranstaltungen.</p>"
    }
];
            ...
            <div class="bg-white shadow-sm hover:shadow-md transition-shadow group flex flex-col fade-in cursor-pointer overflow-hidden rounded-xl border border-gray-100" onclick="window.location.href='shop-items/product?id=${p.id}'">
                ...
                <div class="p-6 flex flex-col flex-grow">
                    ...
                    <div class="flex justify-between items-center mt-auto pt-4 border-t border-gray-100">
                        ${(() => {
                            if (p.priceMode === 'from') {
                                return `<span class="text-gold font-serif text-xl font-medium"><span class="text-xs text-gray-500 mr-1 font-normal">ab</span>${p.price} €</span>`;
                            }
                            const isReq = p.priceMode === 'request' || String(p.price) === '0' || p.price === 'Auf Anfrage' || p.price === 'On Request' || p.price === 'La Cerere';
                            if (isReq) {
                                return '<span class="text-gold font-serif text-base font-semibold">Auf Anfrage</span>';
                            }
                        ...
                        <div class="flex space-x-3 items-center">
                            ...
                            ${(() => {
                                ...
                                if (isReq) {
                                    return `
                                    <button onclick="event.stopPropagation(); window.openProductInquiryModal('${p.id}', 'shop')" class="bg-gold hover:bg-black text-white rounded-full w-8 h-8 flex items-center justify-center transition-colors shadow-sm" title="Anfrage senden">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path></svg>
                                    </button>`;
                                }
                                return `
                                <button onclick="event.stopPropagation(); addToCart('${p.id}')" class="bg-gray-900 hover:bg-gold text-white rounded-full w-8 h-8 flex items-center justify-center transition-colors shadow-sm" title="In den Warenkorb">
...
const checkoutAuthPrompt = document.getElementById('checkout-auth-prompt');
const checkoutPersonalFields = document.getElementById('checkout-personal-fields');
...
if (checkoutForm) {
    checkoutForm.addEventListener('submit', async (e) => {
        ...
        const sameAddrEl = document.getElementById('checkout-same-address');
        const diffAddrEl = document.getElementById('checkout-diff-address');
        const diffPlzEl = document.getElementById('checkout-diff-postcode');
        const diffCityEl = document.getElementById('checkout-diff-city');
        let address = document.getElementById('checkout-address').value;
        let postcode = document.getElementById('checkout-postcode') ? document.getElementById('checkout-postcode').value : '';
        if (sameAddrEl && !sameAddrEl.checked && diffAddrEl && diffAddrEl.value.trim()) {
            address = diffAddrEl.value.trim() + (diffCityEl && diffCityEl.value.trim() ? ', ' + diffCityEl.value.trim() : '');
            if (diffPlzEl && diffPlzEl.value.trim()) postcode = diffPlzEl.value.trim();
        }
        ...
        try {
            ...
            checkoutSubmit.textContent = "Leite zu Stripe weiter...";
            ...
            const errClose = document.getElementById('error-modal-close');
            if (errModal && errMsg && errClose) {
                errMsg.textContent = "Fehler beim Senden: Bitte versuchen Sie es später erneut. (" + error.message + ")";
                errModal.classList.remove('hidden');
                errClose.onclick = () => errModal.classList.add('hidden');
            } else {
...
function showWishlistModal(title, msg, type = "success", redirectUrl = null) {
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            <div class="absolute inset-0 bg-black/60 backdrop-blur-sm transition-opacity" onclick="closeWishlistModal()"></div>
            <div class="relative bg-white w-full max-w-sm mx-4 rounded-2xl shadow-2xl p-8 text-center transform transition-all border-t-4">
                <div id="cw-modal-icon-container" class="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                    <svg id="cw-modal-icon" class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"></svg>
                </div>
                <h3 id="cw-modal-title" class="text-xl font-serif text-gray-900 mb-2"></h3>
                <p id="cw-modal-msg" class="text-gray-500 text-sm mb-6"></p>
                <button id="cw-modal-btn" class="btn-gold w-full justify-center text-center py-3 text-sm tracking-widest block font-medium">OK</button>
            </div>
        `;
        document.body.appendChild(modal);
        
        window.closeWishlistModal = function() {
            document.getElementById('custom-wishlist-modal').classList.add('hidden');
            if (window.cwModalRedirect) {
                window.location.href = window.cwModalRedirect;
            }
        };
        document.getElementById('cw-modal-btn').addEventListener('click', window.closeWishlistModal);
    }

    const modalInner = modal.querySelector('.relative');
    const iconContainer = document.getElementById('cw-modal-icon-container');
    const icon = document.getElementById('cw-modal-icon');
    
    if (type === "success") {
        modalInner.classList.remove('border-red-500', 'border-gold');
        modalInner.classList.add('border-green-500');
        iconContainer.className = "w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4";
        icon.classList.value = "w-8 h-8 text-green-500";
        icon.innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>';
    } else if (type === "error") {
        modalInner.classList.remove('border-green-500', 'border-gold');
        modalInner.classList.add('border-red-500');
        iconContainer.className = "w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4";
        icon.classList.value = "w-8 h-8 text-red-500";
    ...
    document.getElementById('cw-modal-msg').textContent = msg;
    window.cwModalRedirect = redirectUrl;
    
    if (redirectUrl) {
        document.getElementById('cw-modal-btn').textContent = "ZUM LOGIN";
    } else {
        document.getElementById('cw-modal-btn').textContent = "OK";
    }
...
async function addToWishlist(id) {
    if (!window.currentUser) {
        const isSubdir = window.location.pathname.includes('/shop-items/') || window.location.pathname.includes('/verleih-items/');
        const loginPath = (isSubdir ? '../' : '') + 'login.html';
        showWishlistModal("Nicht eingeloggt", "Bitte loggen Sie sich ein, um Artikel zu Ihrer Wunschliste hinzuzufügen.", "error", loginPath);
        return;
    }
    ...
    const existingItem = window.userWishlist ? window.userWishlist.find(item => item.id == id && item.type == currentSource) : null;

    try {
        const userRef = window.firebaseDoc(window.firebaseDb, 'users', window.currentUser.uid);
        
        if (existingItem) {
            await window.firebaseSetDoc(userRef, {
                wishlist: window.firebaseArrayRemove(existingItem)
            }, { merge: true });
            
            window.userWishlist = window.userWishlist.filter(item => !(item.id == id && item.type == currentSource));
            showWishlistModal("Entfernt", `${product.title} wurde von Ihrer Wunschliste entfernt.`, "success");
        } else {
            const productWithType = { ...product, type: currentSource };
            await window.firebaseSetDoc(userRef, {
                wishlist: window.firebaseArrayUnion(productWithType)
            }, { merge: true });
            
            if (!window.userWishlist) window.userWishlist = [];
            window.userWishlist.push(productWithType);
            showWishlistModal("Erfolgreich", `${product.title} wurde zu Ihrer Wunschliste hinzugefügt.`, "success");
        }
        ...
        showWishlistModal("Fehler", "Fehler beim Speichern: " + error.message, "error");
    }
}
